...
## An ongoing project, using Git and Bookdown.

This is not meant as a finished book, but rather an ongoing project, to which we'll add chapters and hopefully convince others to edit or add chapters too. If you have comments, feel free to add them as an issue, or even fork a copy and make changes yourself. It's released under a [Creative Commons Attribution 4.0 licence](https://creativecommons.org/licenses/by/4.0/), which means you can do whatever you like with it as long as you credit the original project team. Some chapters are more finished-looking than others: check back and hopefully these will improve over time, or you can always pitch in and make changes yourself.

The book itself has been created using [Bookdown](https://bookdown.org), a package for R-Studio which helps you to create book-length publications from a group of R-Markdown files. It's also available on Github.
...
R relies on lots of additional packages for full functionality, and you'll need to install these by using the function ```install.packages()```, folled by the package name, in inverted commas. I recommend doing this to install the tidyverse suite of packages which you will end up using all the time. 
