...
```{r}
shp_tidy = shp_tidy %>% left_join(county_totals, 
...
```{r}
ggplot() + 
  geom_sf(data = england_shp %>% left_join(county_totals, by = c('NAME' = 'general_area_of_coverage')), color = 'black', aes(fill = n))
```
...
Unfortunately, the list of counties in the title list doesn't _exactly_ match those in the shapefile. This is becuase county names in the UK are a complicated business and they have revised over time. There's no easy solution to this, because new counties don't always match the borders of the changed county, and mergers might not always just join two old counties together. 
...
And here's the counties in the shapefile not present in the title list:
...
anti_join(shp_tidy %>% 
            ...
            filter(country_of_publication == 'England') %>% 
            group_by(general_area_of_coverage) %>% 
            tally(), by = c('id' = 'general_area_of_coverage')) 
