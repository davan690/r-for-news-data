...
ggplot() + geom_polygon(data = map, aes(x = long, y = lat, group = group), fill = "gray90", color = 'gray20') + coord_fixed(1.3, xlim = c(-10, 2), ylim = c(50,60)) + geom_point(data = title_list %>% mutate(title_id = str_pad(title_id, width = 9, pad = '0')) %>% 
...
long_title_list %>% 
  ...
  tally() %>% 
  ggplot() + geom_bar(aes(x = decade, y = n, fill = JISC), stat = 'identity') + 
  theme_minimal()
```
