...
Another strong advantage of R is the ease at which you can make maps, for visualisation and also more advanced analysis. Using an openly available list of British Library titles, with some additional coordinate information, it's possible to very quickly make high-quality, publishable maps. 

[](title_list_map.png) [](choropleth.png)
...
ggplot() + 
  ...
  labs(title = "Newspapers Held by the British\nLibrary, Great Britain") + 
...
Use st_join to get county information
...
```{r}
eng_1891 = st_read("EW1891_regcounties/EW1891_regcounties.shp")

```

```{r}
scot_1890 = st_read("Spre1890_scocounties/Spre1890_scocounties.shp")
```
...
It needs to be transformed into lat/long coordinates, because the coordinates we have are in that format. This is easy with ```st_transform()```. You need to specify a lat/long 'Coordinate Reference System' or CRS. 4326 is a standard one. 


```{r}
eng_1891 = st_transform(eng_1891, crs = 4326)
```

```{r}
scot_1890  = st_transform(scot_1890, crs = 4326)
```
...
```{r}
gb1891 = rbind(eng_1891, scot_1890 %>%
                 select(-UL_AUTH))
```

## Download and merge the title list with a set of coordinates.

Next, load and pre-process the provided set of coordinates:
...
Change some more column names, select just the relevant columns, change the NA values and get rid of any empty entries.
